...
async function loadTransportData() {
  try {
    const response = await fetch('https://codd-production.up.railway.app/api/transport?limit=6');
    if (!response.ok) {
...
async function refreshTransportData() {
  ...
  try {
    const response = await fetch('https://codd-production.up.railway.app/api/transport?limit=10');
    if (!response.ok) {
